...
//este método no lo usa mi clase, sin embargo me sirve como guía para elaborar el insertarOrdenado, no uso este método para nada.
ListaO& ListaO::insertar( Iterador & i, int elemento ){
   Iterador inicio = this->begin();
   Iterador fin = this->end();
   if(i==inicio){
      this->push_front(elemento);
